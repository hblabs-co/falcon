...
func parseSomiDate(s string) string {
	...
	if out, ok := parseMonthPhrase(s); ok {
		return out
	}
...
// stub the clock when asserting "Ende Mai" → year-inference behavior.
var nowFunc = time.Now

// germanMonths maps lowercase German month names (full and common
// abbreviations) to their 1-12 integer. Used by parseMonthPhrase.
...
// parseMonthPhrase resolves "Anfang|Mitte|Ende <Month> [YYYY]" into a
...
//
// When no year is given, we infer: current year if the month is ≥ the
// current month, otherwise next year. That matches how freelancers read
// these phrases — "ends in January" posted in April means next January.
// Returns ("", false) if s doesn't match this shape.
func parseMonthPhrase(s string) (string, bool) {
